...
class Policy_net:
    def __init__(self, name: str, env):
        ...
        with tf.compat.v1.variable_scope(name):
            ...
            with tf.compat.v1.variable_scope('policy_net'):
                ...
                layer_3 = tf.compat.v1.layers.dense(inputs=layer_2, units=act_space.n, activation=tf.tanh)
                self.act_probs = tf.compat.v1.layers.dense(inputs=layer_3, units=act_space.n, activation=tf.nn.softmax)
            ...
            self.act_stochastic = tf.compat.v1.multinomial(tf.math.log(self.act_probs), num_samples=1)
    ...
    def get_action_prob(self, obs):
        return tf.compat.v1.get_default_session().run(self.act_probs, feed_dict={self.obs: obs})
