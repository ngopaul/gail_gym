...
class Discriminator:
    def __init__(self, env):
        ...
        with tf.compat.v1.variable_scope('discriminator'):
            ...
            expert_a_one_hot += tf.random.normal(tf.shape(input=expert_a_one_hot), mean=0.2, stddev=0.1, dtype=tf.float32)/1.2
            ...
            # add noise for stabilise training
            agent_a_one_hot += tf.random.normal(tf.shape(input=agent_a_one_hot), mean=0.2, stddev=0.1, dtype=tf.float32)/1.2
            ...
            with tf.compat.v1.variable_scope('loss'):
                loss_expert = tf.reduce_mean(input_tensor=tf.math.log(tf.clip_by_value(prob_1, 0.01, 1)))
                loss_agent = tf.reduce_mean(input_tensor=tf.math.log(tf.clip_by_value(1 - prob_2, 0.01, 1)))
            ...
            self.rewards = tf.math.log(tf.clip_by_value(prob_2, 1e-10, 1))  # log(P(expert|s,a)) larger is better for agent

    def construct_network(self, input):
        layer_1 = tf.compat.v1.layers.dense(inputs=input, units=20, activation=tf.nn.leaky_relu, name='layer1')
        layer_2 = tf.compat.v1.layers.dense(inputs=layer_1, units=20, activation=tf.nn.leaky_relu, name='layer2')
        layer_3 = tf.compat.v1.layers.dense(inputs=layer_2, units=20, activation=tf.nn.leaky_relu, name='layer3')
        prob = tf.compat.v1.layers.dense(inputs=layer_3, units=1, activation=tf.sigmoid, name='prob')
        return prob
